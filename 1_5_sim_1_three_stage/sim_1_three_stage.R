...
sd_p1 = 1; sd_p2 = 4; sd_p3 = 2; sd_t1 = 4; sd_t2 = 2; sd_t3 = 1; 
mu_diff = 0} 
if (ind==2){mu_p1 = 0; mu_p2 = 0.7; mu_p3 = 1; 
sd_p1 = 4; sd_p2 = 2; sd_p3 = 1; sd_t1 = 1; sd_t2 = 4; sd_t3 = 2; 
mu_diff = 0} 
...
# for (itt in 1:n.itt){
  ...
  w.IPTW.vec = c((n_p1+n_t1)/(n_p1+n_t1 + n_p2+n_t2 + n_p3+n_t3),
                 (n_p2+n_t2)/(n_p1+n_t1 + n_p2+n_t2 + n_p3+n_t3),
                 (n_p3+n_t3)/(n_p1+n_t1 + n_p2+n_t2 + n_p3+n_t3)
                 )

  w1.e3.fit = m2.func(w.IPTW.vec)
  
  ##
  # test = (sum(x_t1)*(n_t1+n_p1)/n_t1 +
  #   sum(x_t2)*(n_t2+n_p2)/n_t2 + 
  #   sum(x_t3)*(n_t3+n_p3)/n_t3)/(n_p1+n_t1 + n_p2+n_t2 + n_p3+n_t3) -
  #   (sum(x_p1)*(n_t1+n_p1)/n_p1 +
  #      sum(x_p2)*(n_t2+n_p2)/n_p2 + 
  #      sum(x_p3)*(n_t3+n_p3)/n_p3)/(n_p1+n_t1 + n_p2+n_t2 + n_p3+n_t3)
...
results.mat[ind, ] = c(
  mu_p1, mu_p2, mu_p3, sd_p1, sd_t1, mu_diff, 
