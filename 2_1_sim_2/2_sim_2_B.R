...
# for (itt in 1:n.itt){
  ...
  data.reg = data.frame("y" = c(x_p1, x_p2, x_t1, x_t2),
                        "grp" = c(rep(0, n_p1+n_p2), 
                                  rep(1, n_t1+n_t2)),
                        "drift_1" = c(rep(0, n_p1), rep(1, n_p2),
  ...
  m1.est = mean(c(x_t1, x_t2)) - 
    mean(c(x_p1, x_p2))
  m1.se = sqrt((var(c(x_t1, x_t2))/(n_t1+n_t2))+
  ...
  w.IPTW.vec = c((n_p1+n_t1)/(n_p1+n_t1+n_p2+n_t2),
                 (n_p2+n_t2)/(n_p1+n_t1+n_p2+n_t2)
  )
