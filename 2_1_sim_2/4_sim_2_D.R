...
m2.func = function(w.vec.in){
  ...
  m2.est.in = w1.in*(mean(c(x_t3)) - mean(c(x_p3))) + 
    w2.in*(mean(c(x_t4)) - mean(c(x_p4)))
  
  m2.p.val.in = 1-pnorm(m2.est.in/sqrt(w1.in^2*var(x_t3)/n_t3+
                                         w1.in^2*var(x_p3)/n_p3+
                                         w2.in^2*var(x_t4)/n_t4+
                                         w2.in^2*var(x_p4)/n_p4
                                         ))
...
# for (itt in 1:n.itt){
  ...
  ## regression
  time.1 = Sys.time()
  data.reg = data.frame("y" = c(x_p3, x_p4, x_t3, x_t4),
                        "grp" = c(rep(0, n_p3+n_p4), 
                                  rep(1, n_t3+n_t4)),
                        "drift_1" = c(rep(0, n_p3), rep(1, n_p4),
                                      rep(0, n_t3), rep(1, n_t4)),
                        "true_w" = c(rep(1/sd_p3^2, n_p3), 
                                     rep(1/sd_p4^2, n_p4),
                                     rep(1/sd_t3^2, n_t3), 
                                     rep(1/sd_t4^2, n_t4))
  )
  ...
  m1.est = mean(c(x_t3, x_t4)) - 
    mean(c(x_p3, x_p4))
  m1.se = sqrt((var(c(x_t3, x_t4))/(n_t3+n_t4))+
                 (var(c(x_p3, x_p4))/(n_p3+n_p4)))
  m1.p.val = 1-pnorm(m1.est / m1.se)
 
  ########################################################################
  #### method 2: stratified
  ## weight with known variance, optimal unknown weight
  w.true.1 = 1/(sd_t3^2/n_t3+sd_p3^2/n_p3)
  w.true.2 = 1/(sd_t4^2/n_t4+sd_p4^2/n_p4)
  ...
  w.est.1 = 1/(var(x_t3)/n_t3+var(x_p3)/n_p3)
  w.est.2 = 1/(var(x_t4)/n_t4+var(x_p4)/n_p4)
  ...
  w.IPTW.vec = c((n_p3+n_t3)/(n_p4+n_t4+n_p3+n_t3),
                 (n_p4+n_t4)/(n_p4+n_t4+n_p3+n_t3)
  )
