...
m2.func = function(w.vec.in){
  ...
  m2.p.val.in = 1-pnorm(m2.est.in/sqrt(w1.in^2*var(x_t1)/n_t1+
                                         ...
                                         w4.in^2*var(x_t4)/n_t4+
                                         w4.in^2*var(x_p4)/n_p4
                                         ))
...
# for (itt in 1:n.itt){
  ...
  ## regression
  time.1 = Sys.time()
  data.reg = data.frame("y" = c(x_p1, x_p2, x_p3, x_p4, x_t1, x_t2, x_t3, x_t4),
                        "grp" = c(rep(0, n_p1+n_p2+n_p3+n_p4), 
                                  rep(1, n_t1+n_t2+n_t3+n_t4)),
                        "drift_1" = c(rep(0, n_p1), rep(1, n_p2), rep(0, n_p3),rep(0, n_p4),
                                      rep(0, n_t1), rep(1, n_t2), rep(0, n_t3),rep(0, n_t4)),
                        "drift_2" = c(rep(0, n_p1), rep(0, n_p2), rep(1, n_p3),rep(0, n_p4),
                                      rep(0, n_t1), rep(0, n_t2), rep(1, n_t3),rep(0, n_t4)),
                        "drift_3" = c(rep(0, n_p1), rep(0, n_p2), rep(0, n_p3),rep(1, n_p4),
                                      rep(0, n_t1), rep(0, n_t2), rep(0, n_t3),rep(1, n_t4)),
  ...
  lm.fit.2 = lm(y~grp+drift_1 + drift_2+drift_3, data = data.reg, weights = true_w)
  ...
  m1.est = mean(c(x_t1, x_t2, x_t3, x_t4)) - 
    mean(c(x_p1, x_p2, x_p3, x_p4))
  m1.se = sqrt((var(c(x_t1, x_t2, x_t3, x_t4))/(n_t1+n_t2+n_t3+n_t4))+
                 (var(c(x_p1, x_p2, x_p3, x_p4))/(n_p1+n_p2+n_p3+n_p4)))
  m1.p.val = 1-pnorm(m1.est / m1.se)
 
  ########################################################################
  #### method 2: stratified
  ## weight with known variance, optimal unknown weight
  w.true.1 = 1/(sd_t1^2/n_t1+sd_p1^2/n_p1)
  w.true.2 = 1/(sd_t2^2/n_t2+sd_p2^2/n_p2)
  w.true.3 = 1/(sd_t3^2/n_t3+sd_p3^2/n_p3)
  w.true.4 = 1/(sd_t4^2/n_t4+sd_p4^2/n_p4)
  
  w.true.vec = c(w.true.1/(w.true.1+w.true.2+w.true.3+w.true.4),
                 w.true.2/(w.true.1+w.true.2+w.true.3+w.true.4),
                 w.true.3/(w.true.1+w.true.2+w.true.3+w.true.4))
  ...
  w.est.4 = 1/(var(x_t4)/n_t4+var(x_p4)/n_p4)
  
  w.est.vec = c(w.est.1/(w.est.1+w.est.2+w.est.3+w.est.4),
                 w.est.2/(w.est.1+w.est.2+w.est.3+w.est.4),
                 w.est.3/(w.est.1+w.est.2+w.est.3+w.est.4))
  w1.e2.fit = m2.func(w.est.vec)
   
  ## weight that is identical to IPTW
  w.IPTW.vec = c((n_p1+n_t1)/(n_p1+n_t1+n_p2+n_t2+n_p3+n_t3+n_p4+n_t4),
                 (n_p2+n_t2)/(n_p1+n_t1+n_p2+n_t2+n_p3+n_t3+n_p4+n_t4),
                 (n_p3+n_t3)/(n_p1+n_t1+n_p2+n_t2+n_p3+n_t3+n_p4+n_t4),
                 (n_p4+n_t4)/(n_p1+n_t1+n_p2+n_t2+n_p3+n_t3+n_p4+n_t4)
  )
  
  w1.e3.fit = m2.func(w.IPTW.vec)
  
  # ##
  # test = (sum(x_t1)*(n_t1+n_p1)/n_t1+sum(x_t2)*(n_t2+n_p2)/n_t2+
  #           sum(x_t3)*(n_t3+n_p3)/n_t3+sum(x_t4)*(n_t4+n_p4)/n_t4)/
  #   (n_p1+n_t1+n_p2+n_t2+n_p3+n_t3+n_p4+n_t4) - 
  #   (sum(x_p1)*(n_t1+n_p1)/n_p1+sum(x_p2)*(n_t2+n_p2)/n_p2+
  #      sum(x_p3)*(n_t3+n_p3)/n_p3+sum(x_p4)*(n_t4+n_p4)/n_p4)/
  #   (n_p1+n_t1+n_p2+n_t2+n_p3+n_t3+n_p4+n_t4)
...
write.csv(results.mat, "sim2_results.csv")
