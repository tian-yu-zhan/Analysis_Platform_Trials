...
setwd("~/Dropbox/Research/AbbVie/Ratio_master_protocol/code/v2/1_4_sim_1_drift/")
...
m2.func = function(w1.in){
  ...
  m2.p.val.in = 1-pnorm(m2.est.in/sqrt(w1.in^2*var(x_t1)/n_p/ratio_1+
...
if (ind==1){mu_t = 0; mu_add = 0.3; ratio_2 = 0.5; sd_p1 = 1; sd_p2 = 1; 
sd_t1 = 4; sd_t2 = 4; mod_ind = 1}  
if (ind==2){mu_t = 0; mu_add = 0.3; ratio_2 = 0.5; sd_p1 = 1; sd_p2 = 2; 
sd_t1 = 2; sd_t2 = 3; mod_ind = 1} 
if (ind==3){mu_t = 0; mu_add = 0.3; ratio_2 = 0.5; sd_p1 = 1; sd_p2 = 1; 
sd_t1 = 4; sd_t2 = 4; mod_ind = 2}  
if (ind==4){mu_t = 0; mu_add = 0.3; ratio_2 = 0.5; sd_p1 = 1; sd_p2 = 2; 
sd_t1 = 2; sd_t2 = 3; mod_ind = 2} 

if (ind==5){mu_t = 0.8; mu_add = 0.3; ratio_2 = 0.5; sd_p1 = 1; sd_p2 = 1; 
sd_t1 = 4; sd_t2 = 4; mod_ind = 1}  
if (ind==6){mu_t = 0.5; mu_add = 0.3; ratio_2 = 0.5; sd_p1 = 1; sd_p2 = 2; 
sd_t1 = 2; sd_t2 = 3; mod_ind = 1}  
if (ind==7){mu_t = 0.8; mu_add = 0.3; ratio_2 = 0.5; sd_p1 = 1; sd_p2 = 1; 
sd_t1 = 4; sd_t2 = 4; mod_ind = 2}  
if (ind==8){mu_t = 0.5; mu_add = 0.3; ratio_2 = 0.5; sd_p1 = 1; sd_p2 = 2; 
sd_t1 = 2; sd_t2 = 3; mod_ind = 2}  
...
# for (itt in 1:n.itt){
  ...
  if (mod_ind==1){
      mu_s1 = (1:(n_p+n_t1))*0.0005
      mu_s2 = (1:(n_p+n_t2))*0.0005 + 0.2
  } else if (mod_ind==2){
      mu_s1 = (1:(n_p+n_t1))^2*0.000005
      mu_s2 = (1:(n_p+n_t2))^2*0.000002 + 0.2
  }

    
    # seq(from = 0, to = mu_add, length.out = n_p + n_t1)
  ind_s1_p = sort(sample(1:(n_p + n_t1), n_p))
  mu_s1_p = mu_s1[ind_s1_p]
  mu_s1_t = mu_s1[-ind_s1_p]
    
    # seq(from = 0.2, to = 0.2+mu_add, length.out = n_p + n_t2)
  ind_s2_p = sort(sample(1:(n_p + n_t2), n_p))
  mu_s2_p = mu_s2[ind_s2_p]
  mu_s2_t = mu_s2[-ind_s2_p]
  
  x_p1 = rnorm(n_p, mu_s1_p, sd_p1)
  x_p2 = rnorm(n_p, mu_s2_p, sd_p2)
  
  x_t1 = rnorm(n_t1, mu_s1_t + mu_t, sd_t1)
  x_t2 = rnorm(n_t2, mu_s2_p + mu_t, sd_t2)
  
  ## regression
  time.1 = Sys.time()
  data.reg = data.frame("y" = c(x_p1, x_p2, x_t1, x_t2),
                        "grp" = c(rep(0, n_p+n_p), rep(1, n_t1+n_t2)),
                        "drift" = c(rep(0, n_p), rep(1, n_p),
                                    rep(0, n_t1), rep(1, n_t2)),
                        "true_w" = c(rep(1/sd_p1^2, n_p), 
                          rep(1/sd_p2^2, n_p),
                          rep(1/sd_t1^2, n_t1), 
                          rep(1/sd_t2^2, n_t2))
  )
  ...
  data.reg$w = c(rep(1/mean(int.lm.fit$residuals[data.reg$grp==0&data.reg$drift==0]^2), n_p), 
                 rep(1/mean(int.lm.fit$residuals[data.reg$grp==0&data.reg$drift==1]^2), n_p),
  ...
  m1.se = sqrt((var(c(x_t1, x_t2))/(n_p*ratio_1 + n_p*ratio_2))+
                 (var(c(x_p1, x_p2))/(n_p + n_p)))
...
write.csv(results.mat, "sim1_drift_results.csv")
...
data.out.1 = data.frame(
  "scen" = rep(c("S8", "S9", "S10", "S11"), 2), 
  ...
  "m2_tilde" = paste0(sprintf("%.2f", results.mat$bias_m2_e4*100), " (", 
                    sprintf("%.2f", results.mat$MSE_m2_e4*100), ")"),
  ...
  "wlm_true" = paste0(sprintf("%.2f", results.mat$bias_wlm_true*100), " (", 
                      sprintf("%.2f", results.mat$MSE_wlm_true*100), ")"),
  "lm" = paste0(sprintf("%.2f", results.mat$bias_lm*100), " (", 
                    sprintf("%.2f", results.mat$MSE_lm*100), ")"),
  "wlm" = paste0(sprintf("%.2f", results.mat$bias_wlm*100), " (", 
                sprintf("%.2f", results.mat$MSE_wlm*100), ")")
)

data.out.2 = data.frame(
  "scen" = rep(c("S8", "S9", "S10", "S11"), 2), 
  "mu_t" = results.mat$mu_t,
  "power_m1" = paste0(sprintf("%.2f", results.mat$power_m1*100), "%"),
  "power_m2_IPTW" = paste0(sprintf("%.2f", results.mat$power_m2_e3*100), "%"),
  "power_m2_tilde" = paste0(sprintf("%.2f", results.mat$power_m2_e4*100), "%"),
  "power_m2_hat" = paste0(sprintf("%.2f", results.mat$power_m2_e2*100), "%"),
  "power_m2_opt" = paste0(sprintf("%.2f", results.mat$power_m2_e1*100), "%"),
  "power_wlm_true" = paste0(sprintf("%.2f", results.mat$power_wlm_true*100), "%"),
  "power_lm" = paste0(sprintf("%.2f", results.mat$power_lm*100), "%"),
  "power_wlm" = paste0(sprintf("%.2f", results.mat$power_wlm*100), "%")
)

print(xtable(data.out.1), include.rownames = FALSE)
print(xtable(data.out.2), include.rownames = FALSE)
